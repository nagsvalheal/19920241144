...
export default class BiPspbSymptomTracker extends NavigationMixin(LightningElement) {
    ...
    //             const file = files[i];
    //             const fileAlreadyUploaded = newImageUrls.some(url => url.fileName === file.name);
    ...
    //                         // Handle image files with FileReader for image preview
    ...
    //                         // Handle PDF files by displaying the PDF icon                            
    //                         newImageUrls.push({
    //                             dataUrl: label.PDF_FILE,
    //                             isPdf: true,
    //                             fileName: this.truncateFileName(fileName)
    //                         });
    //                         this.imageUrls = [...newImageUrls];
    ...
    //                     // Check total file size after each upload
    //                     let sum = 0;
    //                     for (let j = 0; j < this.totalSize.length; j++) {
    //                         sum += this.totalSize[j];
    //                         if (sum > maxFileSize) {
    //                             this.upLoadedLarge = true;
    //                             this.totalSize.splice(j, 1); // Remove the oversized file
    //                         }
    //                     }
    //                 } else {
    ...
    //                 }
    //             } else {
    //                 // Not an image or PDF, show error message
    //                 this.invalidFileType = true;
    //                 this.upLoadedLarge = false;
    //             }
    //         }
            ...
            if (data && data !== null) {
                ...
                if (this.firstTime === false) {
                    ...
                    let splitArray = data.map((obj) => {
                        ...
                        return "data:" + obj.split("data:")[1];
                    });
                    splitArray.forEach(record => {
                        if (record !== "" && record !== undefined) {
                            if (!this.oldimageurl.some(oldUrl => oldUrl.dataUrl === record)) {

                                const isPdf = record.includes("application/pdf");
                                const fileName = this.extractFileName(record);
                                newImageUrls.push({
                                    dataUrl: record,
                                    isPdf: isPdf,
                                    fileName: this.truncateFileName(fileName),
    ...
    handleFileInputChange(event) {
        ...
        Array.from(files).forEach(file => {
            ...
            if (this.isValidFileType(file) && this.checkFileSize(file, maxFileSize)) {
    ...
    handleClickpdf() {
        ...
        if (parentId) {
            this.saveFiles(fileContents, parentId, this.fileNmaedoc);
        }
    }
    ...
    saveFiles(fileContents, parentId, fileName) {
        SAVE_FILES({ fileContents, parentId, fileName })
            .then(attachmentIds => {
                this.attachmentIdsValues = attachmentIds;
                return CREATE_CONTENT_DOCUMENT_LINKS({ fileIds: this.attachmentIdsValues, symptomTrackerId: parentId });
            })
            .then(() => {
